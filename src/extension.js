...
var zestySDK = null;
var basePath = '';
var folders = ['/webengine', '/webengine/views', '/webengine/styles', '/webengine/scripts'];
var zestyConfig = {};
var token = '';
...
async function validate() {
  ...
  if (res.error) {
    vscode.window.showErrorMessage('Invalid or expired developer token provided.');
    return false;
  }
  return true;
}
...
async function init() {
  ...
  if (token && zestyConfig.instance_zuid) zestySDK = new sdk(zestyConfig.instance_zuid, token);
}
...
function makeFileSync(type, filename, content) {
  try {
    var file = basePath;
    if (type === 'view')
      file += `${folders[1]}/${filename.charAt(0) !== '/' ? filename : filename.substring(1)}`;
  ...
  res.data
    ...
    .forEach((script) => {
      ...
      scriptObj[script.fileName] = {
        ...
        lastSyncedAt: script.updatedAt || script.updated_at || script.createdAt || script.created_at
      };
    });
...
function isFileSaveSyncEnabled() {
  const fileSaveConfig = vscode.workspace.getConfiguration('zesty.editor').get('syncFileOnSave');
...
async function saveFile(document) {
  ...
  if (remote.code && remote.code !== code) {
    const remoteChanged = isRemoteNewerThanLastSync(remote.updatedAt, file.instance);
    if (remoteChanged) {
  ...
  switch (file.extension) {
    ...
    case 'sass':
      {
        const res = await zestySDK.instance.updateStylesheet(file.instance.zuid, payload);
        updatedAt = res && res.data ? res.data.updatedAt : null;
      }
      vscode.window.showInformationMessage(`Saving stylesheet to ${file.instance.zuid}.`);
      break;
    case 'js':
      {
        const res = await request(
      ...
      vscode.window.showInformationMessage(`Saving script to ${file.instance.zuid}.`);
      break;
    default:
      {
      ...
      vscode.window.showInformationMessage(`Saving view to ${file.instance.zuid}.`);
      break;
  }
...
function isRemoteNewerThanLastSync(remoteUpdatedAt, instance) {
  ...
  const lastSyncedAt = (instance && (instance.lastSyncedAt || instance.updatedAt)) || null;
...
async function showDiff(remoteCode, document, file) {
  ...
  await vscode.commands.executeCommand('vscode.diff', remoteDoc.uri, document.uri, title);
}
...
async function pullPublishedFileFromUri(uri) {
  ...
  await vscode.workspace.fs.writeFile(document.uri, buffer);
  await updateLastSyncedAt(file, remote.updatedAt);
  vscode.window.showInformationMessage('Pulled published (live) file from instance.');
}
...
async function publishFileFromUri(uri) {
  ...
  switch (file.extension) {
    ...
    case 'sass':
      if (!remote.version) {
        vscode.window.showErrorMessage('Unable to determine stylesheet version to publish.');
        return;
      }
      await zestySDK.instance.publishStylesheet(file.instance.zuid, remote.version);
      vscode.window.showInformationMessage(`Published stylesheet ${file.filename}.`);
      break;
    case 'js':
      await request(
      ...
      vscode.window.showInformationMessage(`Published script ${file.filename}.`);
      break;
    default:
      if (!remote.version) {
        vscode.window.showErrorMessage('Unable to determine view version to publish.');
        return;
      }
...
function getFile(file) {
  ...
  var newSplitPath = splitPath.slice(splitPath.indexOf('webengine'), splitPath.length);
  if (newSplitPath[0] === 'webengine') newSplitPath.shift();
  if (['styles', 'scripts', 'views'].includes(newSplitPath[0])) newSplitPath.shift();
...
async function activate(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('zesty-vscode-extension.syncFile', async (uri) => {
      await syncFileFromUri(uri);
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zesty-vscode-extension.saveAndSyncFile', async (uri) => {
      await syncFileFromUri(uri, { forceSave: true });
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zesty-vscode-extension.pullFile', async (uri) => {
      await pullFileFromUri(uri);
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zesty-vscode-extension.pullPublishedFile', async (uri) => {
      await pullPublishedFileFromUri(uri);
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('zesty-vscode-extension.publishFile', async (uri) => {
  ...
  vscode.workspace.onDidDeleteFiles(async (event) => {
    ...
    if (event.files) {
      ...
      if (event.files.length > 1) {
        vscode.window.showErrorMessage(`Multiple file deletion is not yet supported.`);
        return;
      }
      if (!file.instance) {
        vscode.window.showErrorMessage('Cannot sync to the instance.');
      ...
      switch (file.extension) {
        ...
        case 'sass':
          ...
          await writeConfig();
          vscode.window.showInformationMessage(`Deleting stylesheet from ${file.instance.zuid}`);
          break;
        case 'js':
          await request(
            `https://${zestyConfig.instance_zuid}.api.zesty.io/v1/web/scripts/${file.instance.zuid}`,
          ...
          await writeConfig();
          vscode.window.showInformationMessage(`Deleting script from ${file.instance.zuid}`);
          break;
        default:
          await request(
          ...
          await writeConfig();
          vscode.window.showInformationMessage(`Deleting view from ${file.instance.zuid}`);
          break;
      }
    }
  });
      ...
      switch (file.extension) {
        ...
        case 'sass':
          ...
          if (!resStyle.error) {
            ...
            await writeConfig();
            vscode.window.showInformationMessage(`Creating stylesheet to ${resStyle.data.ZUID}.`);
          }
          break;
        case 'js':
          ...
          if (!resScript.error) {
            ...
            await writeConfig();
            vscode.window.showInformationMessage(`Creating script to ${resScript.data.ZUID}.`);
          }
          break;
        case undefined:
          ...
          if (!resSnippet.error) {
            ...
            await writeConfig();
            vscode.window.showInformationMessage(`Creating file to ${resSnippet.data.ZUID}.`);
          }
          break;
        default:
          ...
          if (!resCustom.error) {
            ...
            await writeConfig();
            vscode.window.showInformationMessage(`Creating file to ${resCustom.data.ZUID}.`);
          }
          break;
      }
    }
