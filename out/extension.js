...
const zestyManager_1 = require("./src/zestyManager");
const zestySnapshots_1 = require("./src/zestySnapshots");
class WebengineTreeNode extends vscode.TreeItem {
    constructor(kind, label, collapsibleState, uri, workspaceFolder) {
...
class WebengineDetailsNode extends vscode.TreeItem {
    constructor(kind, label, collapsibleState, children) {
        super(label, collapsibleState);
        this.kind = kind;
...
class WebengineFileDetailsProvider {
    ...
    async getChildren(element) {
        ...
        const managerUri = this.details.resource.record?.zuid
            ...
            : undefined;
        if (managerUri) {
            sections.push(this.createSectionNode('Actions', [
        ...
        summaryItems.push(this.createItemNode('Instance ZUID', this.details.instanceZuid ?? 'Missing zesty.config.json value'));
        const currentVersion = readNumberLike(this.details.currentData, ['version', 'version_num', 'versionNumber', 'meta.version']);
        const publishedVersion = readNumberLike(this.details.publishedData, ['version', 'version_num', 'versionNumber', 'meta.version']);
        ...
        summaryItems.push(this.createItemNode('Created At', readStringLike(this.details.currentData, ['createdAt', 'created_at']) ??
            readStringLike(this.details.resource.record, ['createdAt']) ??
            'Unavailable'));
        summaryItems.push(this.createItemNode('Updated At', readStringLike(this.details.currentData, ['updatedAt', 'updated_at']) ??
            readStringLike(this.details.resource.record, ['updatedAt']) ??
            'Unavailable'));
        summaryItems.push(this.createItemNode('Last Synced', this.details.resource.record?.lastSyncedAt ?? 'Unavailable'));
        ...
        const publishingNodes = this.details.publishedVersions.length
            ? this.details.publishedVersions.map((entry) => {
                ...
                const item = this.createItemNode(label, `v${version ?? '?'}${updatedAt ? ` • ${updatedAt}` : ''}`);
    ...
    createSectionNode(label, children) {
        const section = new WebengineDetailsNode('section', label, vscode.TreeItemCollapsibleState.Expanded, children);
        section.contextValue = 'zestyWebengineDetailsSection';
        section.iconPath = new vscode.ThemeIcon('list-tree');
        return section;
    }
    createItemNode(label, value) {
        const node = new WebengineDetailsNode('item', `${label}: ${String(value)}`, vscode.TreeItemCollapsibleState.None);
        node.contextValue = 'zestyWebengineDetailsItem';
        node.iconPath = new vscode.ThemeIcon('circle-small-filled');
        return node;
    }
    createActionNode(label, options) {
    ...
    createMessageNode(message) {
        const node = new WebengineDetailsNode('message', message, vscode.TreeItemCollapsibleState.None);
        node.contextValue = 'zestyWebengineDetailsMessage';
        node.iconPath = new vscode.ThemeIcon('info');
        return node;
    }
    async loadDetails(file) {
        ...
        if (currentResponse.ok) {
            ...
            details.warning = `Current file API request failed (${currentResponse.statusCode ?? 'unknown'}): ${currentResponse.error}`;
        }
        if (liveResponse.ok) {
...
class WebengineSidebarProvider {
    ...
    createWorkspaceNode(folder) {
        const webengineUri = vscode.Uri.joinPath(folder.uri, 'webengine');
        const exists = this.pathExists(webengineUri.fsPath);
        const node = new WebengineTreeNode('workspace', folder.name, exists ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None, webengineUri, folder);
    ...
    async getWorkspaceChildren(folder) {
        const webengineUri = vscode.Uri.joinPath(folder.uri, 'webengine');
    ...
    async getFolderChildren(workspaceFolder, folderUri) {
        let entries;
        try {
        ...
        for (const [name, fileType] of entries) {
            ...
            if (fileType === vscode.FileType.Directory) {
                directoryNodes.push(this.createFolderNode(name, childUri, workspaceFolder, vscode.TreeItemCollapsibleState.Collapsed));
                continue;
            }
            fileNodes.push(this.createFileNode(name, childUri, workspaceFolder));
        }
        directoryNodes.sort((a, b) => String(a.label ?? '').localeCompare(String(b.label ?? '')));
        fileNodes.sort((a, b) => String(a.label ?? '').localeCompare(String(b.label ?? '')));
        return [...directoryNodes, ...fileNodes];
    }
    createFolderNode(name, folderUri, workspaceFolder, collapsibleState) {
        const node = new WebengineTreeNode('folder', name, collapsibleState, folderUri, workspaceFolder);
        node.contextValue = 'zestyWebengineFolder';
        node.iconPath = new vscode.ThemeIcon('folder');
        return node;
    }
    createFileNode(name, fileUri, workspaceFolder) {
        const node = new WebengineTreeNode('file', name, vscode.TreeItemCollapsibleState.None, fileUri, workspaceFolder);
    ...
    buildFileTooltip(workspaceFolder, fileUri, metadata) {
        const relative = path
    ...
    resetWatchers() {
        ...
        for (const folder of vscode.workspace.workspaceFolders ?? []) {
            const webengineWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(folder, 'webengine/**'));
            const configWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(folder, 'zesty.config.json'));
            const refresh = () => this.refresh();
            this.watcherDisposables.push(webengineWatcher, configWatcher, webengineWatcher.onDidCreate(refresh), webengineWatcher.onDidDelete(refresh), webengineWatcher.onDidChange(refresh), configWatcher.onDidCreate(refresh), configWatcher.onDidDelete((uri) => {
...
function readWorkspaceZestyConfig(workspaceFolder) {
    const configPath = path.join(workspaceFolder.uri.fsPath, 'zesty.config.json');
...
function resolveWebengineResource(workspaceFolder, fileUri, config) {
...
function lookupWebengineRecord(records, filename, isView) {
    if (!records) {
        return undefined;
    }
    if (isView) {
...
function readStringLike(source, keys) {
...
function readNumberLike(source, keys) {
    for (const key of keys) {
        const value = readPathValue(source, key);
...
function buildPublishedVersions(versions, liveData) {
    ...
    for (const versionEntry of versions) {
        ...
        const hasPublishedStatus = status === 'live' || status === 'published' || status === 'production' || status === 'active';
...
function readBooleanLike(source, keys) {
    for (const key of keys) {
        const value = readPathValue(source, key);
...
function zestyApiGet(instanceZuid, token, endpointPath) {
    return zestyApiRequest(instanceZuid, token, 'GET', endpointPath);
}
function zestyApiRequest(instanceZuid, token, method, endpointPath, payload) {
    return new Promise((resolve) => {
        const request = https.request({
            ...
            response.on('end', () => {
                ...
                const errorMessage = readStringLike(parsedRecord, ['message', 'error']) ??
                    `HTTP ${statusCode}`;
...
const SUPPORTED_LANGUAGE_IDS = new Set([PARSLEY_LANGUAGE_ID, 'html']);
const ROOT_OBJECT_FIELDS = {
    this: [
        'title',
        'content',
        'meta_title',
...
const PREFIXED_VARIABLE_DOC = 'Parsley prefixed variable (`$` local/session or `@` cookie variable). Do not namespace it with `this.`.';
const AUTO_LAYOUT_ALLOWED_MODES = new Set(['auto', 'stacked']);
const KEYWORD_DOCS = {
    ...
    include: 'Include a snippet/view by path. Includes are processed before Parsley evaluation and can use dynamic values.',
...
const METHOD_DOCS = {
    ...
    autoLayout: 'AutoLayout helper. Allowed mode values are `auto` and `stacked`.',
...
const QUALIFIED_METHOD_DOCS = {
    ...
    'instance.searchformatted': 'Formats text for search indexing with optional length limits.',
...
const COMPLETION_BASE_ITEMS = [
    ...
    },
    {
        ...
        documentation: 'Assigns a local variable. Also supported: `$_` for session variables and `@` for cookie variables.',
...
function activate(context) {
    ...
    context.subscriptions.push(diagnostics, webengineSidebarProvider, webengineFileDetailsProvider, webengineTreeView, vscode.window.registerTreeDataProvider('zestyWebengineFileDetails', webengineFileDetailsProvider), webengineTreeView.onDidChangeSelection((event) => {
    ...
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument((document) => runValidation(document)), vscode.workspace.onDidSaveTextDocument((document) => runValidation(document)), vscode.workspace.onDidCloseTextDocument((document) => diagnostics.delete(document.uri)), vscode.workspace.onDidChangeTextDocument((event) => {
        const validateOnType = vscode.workspace
            .getConfiguration('zestyParsley')
            .get('validateOnType', true);
        if (validateOnType) {
            scheduleValidation(event.document);
        }
        maybeAutoCloseBlock(event);
    }));
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider(PARSLEY_SELECTOR, createCompletionProvider(), '.', '{'), vscode.languages.registerHoverProvider(PARSLEY_SELECTOR, createHoverProvider()), vscode.languages.registerDocumentFormattingEditProvider(PARSLEY_LANGUAGE_ID, createFormattingProvider()));
    context.subscriptions.push(vscode.commands.registerCommand('zestyParsley.validateTemplate', async () => {
        ...
        if (!isSupportedDocument(document)) {
            await vscode.window.showWarningMessage('This file type is not supported by Zesty Parsley Tools.');
            return;
        }
        if (!shouldValidateDocument(document)) {
            diagnostics.delete(document.uri);
            await vscode.window.showInformationMessage('No Parsley syntax detected in this document. Validation skipped.');
            return;
        }
        ...
        if (result.length === 0) {
            ...
            await vscode.window.showWarningMessage(`Parsley validation found ${result.length} issue${result.length === 1 ? '' : 's'}.`);
        }
        ...
        void webengineFileDetailsProvider.refresh();
    }), vscode.commands.registerCommand('zestyParsley.webengine.openFile', async (node) => {
        await openWebengineFileLocally(node);
    }), vscode.commands.registerCommand('zestyParsley.webengine.openInZesty', async (node) => {
        await openWebengineFileInZesty(node, false);
    }), vscode.commands.registerCommand('zestyParsley.webengine.createRollbackSnapshot', async (node) => {
        await createRollbackSnapshot(node);
    }), vscode.commands.registerCommand('zestyParsley.webengine.rollbackFile', async (node) => {
        await rollbackWebengineFile(node);
    }), vscode.commands.registerCommand('zestyParsley.webengine.syncFile', async (node) => {
    ...
    }), vscode.commands.registerCommand('zestyParsley.webengine.pullFile', async (node) => {
        const targetUri = resolveWebengineCommandUri(node);
        if (!targetUri) {
    ...
    }), vscode.commands.registerCommand('zestyParsley.webengine.pullPublishedFile', async (node) => {
        const targetUri = resolveWebengineCommandUri(node);
        if (!targetUri) {
            await vscode.window.showWarningMessage('No WebEngine file selected to pull published.');
            return;
        }
        await vscode.commands.executeCommand('zesty-vscode-extension.pullPublishedFile', targetUri);
    }), vscode.commands.registerCommand('zestyParsley.webengine.publishFile', async (node) => {
        const targetUri = resolveWebengineCommandUri(node);
        if (!targetUri) {
...
function resolveWebengineCommandUri(node) {
    return resolveWebengineCommandTarget(node)?.uri;
}
function resolveWebengineCommandTarget(node) {
    if (node &&
        typeof node === 'object' &&
        'kind' in node &&
        node.kind === 'file' &&
...
async function openWebengineFileLocally(node) {
    const targetUri = resolveWebengineCommandUri(node);
    if (!targetUri) {
...
async function openWebengineFileInZesty(node, fallbackToLocal = false) {
    ...
    if (!managerUri) {
        ...
        await vscode.window.showWarningMessage('Unable to build a Zesty manager link for this file. Check zesty.config.json.');
        return;
    }
...
async function rollbackWebengineFile(node) {
    ...
    if (pickItems.length === 0) {
        await vscode.window.showWarningMessage('No rollback snapshots were found for this file. Create a snapshot first.');
        return;
    }
    ...
    if (choice === 'preview') {
        ...
        const secondChoice = await promptRollbackAction(document, context.resource.filename, selection, true);
...
async function createRollbackSnapshot(node) {
    const context = await loadWebengineRollbackContext(node);
    if (!context) {
        return;
    }
    ...
    const archiveFileName = (0, zestySnapshots_1.buildRollbackSnapshotArchiveFileName)(context.resource.type, context.resource.relativePath);
    ...
    if (archiveRecord && !existingSnapshots) {
        await vscode.window.showErrorMessage('Unable to parse the existing rollback snapshot archive for this file.');
        return;
    }
    const archivePayload = [...(existingSnapshots ?? []), snapshot];
    const response = archiveRecord
        ? await zestyApiRequest(context.instanceZuid, context.token, 'PUT', `/web/views/${encodeURIComponent(archiveRecord.snapshotZuid)}`, {
    ...
    if (!response.ok) {
        await vscode.window.showErrorMessage(`Unable to create rollback snapshot in Zesty (${response.statusCode ?? 'unknown'}): ${response.error ?? 'Request failed'}`);
        return;
    }
    await vscode.window.showInformationMessage(`Rollback snapshot "${snapshotName.trim()}" saved to ${archiveFileName}.`);
}
async function applyRollbackChoice(document, rollbackCode, targetUri, action) {
...
async function promptRollbackAction(document, filename, selection, afterPreview = false) {
    ...
    const choice = await vscode.window.showWarningMessage(`${warning} ${detail}`, { modal: true }, 'Preview Diff', 'Rollback in Dev', 'Rollback and Publish');
    switch (choice) {
        case 'Preview Diff':
            return 'preview';
...
async function showRollbackDiff(document, rollbackCode, filename, versionLabel) {
    const historicalDocument = await vscode.workspace.openTextDocument({
        language: document.languageId,
        content: rollbackCode
    });
    await vscode.commands.executeCommand('vscode.diff', historicalDocument.uri, document.uri, `Zesty: ${versionLabel} ↔ Local (${filename})`);
}
async function loadWebengineRollbackContext(node) {
    ...
    if (!resource || !instanceZuid || !fileZuid) {
        await vscode.window.showWarningMessage('This file is not mapped in zesty.config.json. Run "Zesty: Sync Instance Files" first.');
        return undefined;
    }
    if (!token) {
    ...
    if (!versionsResponse.ok) {
        await vscode.window.showErrorMessage(`Unable to load version history from Zesty (${versionsResponse.statusCode ?? 'unknown'}): ${versionsResponse.error ?? 'Request failed'}`);
        return undefined;
    }
...
async function loadRollbackSnapshotPickItems(context) {
    const response = await zestyApiGet(context.instanceZuid, context.token, '/web/views');
    if (!response.ok) {
        await vscode.window.showErrorMessage(`Unable to load rollback snapshots from Zesty (${response.statusCode ?? 'unknown'}): ${response.error ?? 'Request failed'}`);
        return [];
    }
    const archiveFileName = (0, zestySnapshots_1.buildRollbackSnapshotArchiveFileName)(context.resource.type, context.resource.relativePath);
    const prefix = (0, zestySnapshots_1.buildRollbackSnapshotPrefix)(context.resource.type, context.resource.relativePath);
    ...
    if (archiveRecord && !archivedSnapshots) {
        await vscode.window.showErrorMessage('Unable to read the rollback snapshot archive for this file.');
        return [];
    }
    const legacySnapshotEntries = await Promise.all(viewRecords
        .filter((entry) => entry.snapshotFileName !== archiveFileName &&
...
async function loadRollbackSnapshotPayload(context, snapshotZuid) {
    const response = await zestyApiGet(context.instanceZuid, context.token, `/web/views/${encodeURIComponent(snapshotZuid)}`);
    if (!response.ok) {
        return undefined;
    }
    ...
    return (0, zestySnapshots_1.parseRollbackSnapshotPayload)(code);
}
async function loadRollbackSnapshotArchivePayload(context, snapshotZuid) {
    const response = await zestyApiGet(context.instanceZuid, context.token, `/web/views/${encodeURIComponent(snapshotZuid)}`);
    if (!response.ok) {
        return undefined;
    }
    ...
    return (0, zestySnapshots_1.parseRollbackSnapshotArchive)(code);
}
async function findRollbackSnapshotArchiveRecord(context, archiveFileName) {
    const response = await zestyApiGet(context.instanceZuid, context.token, '/web/views');
    if (!response.ok) {
        await vscode.window.showErrorMessage(`Unable to load rollback snapshots from Zesty (${response.statusCode ?? 'unknown'}): ${response.error ?? 'Request failed'}`);
        return undefined;
...
function toRollbackSnapshotViewRecord(entry) {
...
function resolveWebengineManagerUri(node) {
    const target = resolveWebengineCommandTarget(node);
    if (!target) {
        return undefined;
    }
    ...
    return vscode.Uri.parse((0, zestyManager_1.buildZestyManagerUrl)({
        instanceZuid,
        resourceType: resource.type,
        fileZuid
    }));
}
function isWebengineFileUri(uri) {
    if (uri.scheme !== 'file') {
        return false;
    }
        ...
        provideCompletionItems(document, position) {
            ...
            if (propertyMatch && typeof propertyMatch.index === 'number') {
                ...
                for (const field of fields) {
                    ...
                    const item = new vscode.CompletionItem(`${root}.${field}`, isMethodLike ? vscode.CompletionItemKind.Method : vscode.CompletionItemKind.Field);
                    item.detail = `Parsley ${root} ${isMethodLike ? 'helper' : 'field'}`;
                    item.documentation = new vscode.MarkdownString(VARIABLE_DOCS[`${root}.${cleanField}`] ??
            ...
            for (const entry of COMPLETION_BASE_ITEMS) {
                const item = new vscode.CompletionItem(entry.label, entry.kind ?? vscode.CompletionItemKind.Variable);
            ...
            items.push(createSnippetCompletion('if', '{{if ${1:condition}}}\n  $0\n{{/if}}', 'Parsley if block'), createSnippetCompletion('if-else-if', '{{if ${1:condition}}}\n  ${2}\n{{else-if ${3:condition}}}\n  ${4}\n{{else}}\n  ${5}\n{{/if}}', 'Parsley if / else-if / else block'), createSnippetCompletion('each', '{{each ${1:items} as ${2:item} sort by ${3:item.created_at} limit ${4:0},${5:10}}}\n  $0\n{{/each}}', 'Parsley each block with alias/sort/limit'), createSnippetCompletion('include', '{{include ${1:components/header}}}', 'Parsley include helper'), createSnippetCompletion('set-local', '{{$${1:var_name} = ${2:value}}}', 'Set a local Parsley variable'), createSnippetCompletion('set-session', '{{$_${1:session_name} = ${2:value}}}', 'Set a session Parsley variable'), createSnippetCompletion('set-cookie', '{{@${1:cookie_name} = ${2:value}}}', 'Set a cookie Parsley variable'), createSnippetCompletion('autolayout', '{{${1:this}.autoLayout(${2|auto,stacked|})}}', 'AutoLayout with allowed modes'));
            return items;
        }
    };
}
...
function createHoverProvider() {
    return {
        provideHover(document, position) {
            ...
            const token = getIdentifierAtOffset(segment.innerRaw, document.offsetAt(position) - segment.innerStartOffset);
            if (!token) {
                return undefined;
            }
...
function formatParsleyDocument(text, indentSize) {
    ...
    for (const segment of segments) {
        ...
        if (/^if\b/.test(trimmed)) {
            ...
            if (prefix) {
                ...
                validateVariableUsage(document, expressionRaw, segment.innerStartOffset + prefix[0].length, diagnostics, loopAliases);
            }
            continue;
        }
        if (/^each\b/.test(trimmed)) {
            ...
            if (prefix) {
                ...
                validateVariableUsage(document, expressionRaw, segment.innerStartOffset + prefix[0].length, diagnostics, aliasesForExpression, true);
            }
        ...
        if (/^else\b/.test(trimmed)) {
            ...
            if (elseIfPrefix) {
                ...
                if (!top || top.type !== 'if') {
                    diagnostics.push(new vscode.Diagnostic(segmentRange, 'Unexpected {{else-if}} found outside of an open {{if}} block.', vscode.DiagnosticSeverity.Warning));
                }
                else {
                    const expressionRaw = segment.innerRaw.slice(elseIfPrefix[0].length);
                    validateVariableUsage(document, expressionRaw, segment.innerStartOffset + elseIfPrefix[0].length, diagnostics, loopAliases);
                }
                continue;
            }
            const top = blocks[blocks.length - 1];
            if (!top || top.type !== 'if') {
                diagnostics.push(new vscode.Diagnostic(segmentRange, 'Unexpected {{else}} found outside of an open {{if}} block.', vscode.DiagnosticSeverity.Warning));
            }
            continue;
        }
        const closing = /^(?:\/|end[-_])(if|each)\b/.exec(trimmed);
        if (closing) {
            const expected = closing[1];
            const top = blocks.pop();
            if (!top) {
                diagnostics.push(new vscode.Diagnostic(segmentRange, `Closing tag {{/${expected}}} does not have a matching opening block.`, vscode.DiagnosticSeverity.Warning));
                continue;
            }
            if (top.type !== expected) {
                diagnostics.push(new vscode.Diagnostic(segmentRange, `Expected {{/${top.type}}} before {{/${expected}}}.`, vscode.DiagnosticSeverity.Warning));
            }
            continue;
        }
        if (/^include\b/.test(trimmed)) {
            ...
            if (!includeArgs) {
                diagnostics.push(new vscode.Diagnostic(segmentRange, 'Include syntax requires a snippet/view target (for example: {{include components/header}}).', vscode.DiagnosticSeverity.Warning));
            }
            continue;
        }
        ...
        validateVariableUsage(document, segment.innerRaw, segment.innerStartOffset, diagnostics, loopAliases);
    }
    for (const block of blocks) {
        diagnostics.push(new vscode.Diagnostic(block.range, `Unclosed {{${block.type}}} block. Missing {{/${block.type}}}.`, vscode.DiagnosticSeverity.Warning));
    }
    return diagnostics;
}
function validateVariableUsage(document, expressionRaw, expressionStartOffset, diagnostics, allowedIdentifiers = new Set(), allowBareIdentifiers = false) {
    ...
    for (const malformedMatch of expressionRaw.matchAll(malformedDotPath)) {
        ...
        diagnostics.push(new vscode.Diagnostic(offsetsToRange(document, start, end), 'Invalid variable usage: consecutive dots are not allowed in Parsley variables.', vscode.DiagnosticSeverity.Warning));
    }
    for (const match of expressionRaw.matchAll(IDENTIFIER_PATTERN)) {
        ...
        diagnostics.push(new vscode.Diagnostic(offsetsToRange(document, start, end), `Invalid variable usage: "${token}" should be namespaced (for example: this.${token}).`, vscode.DiagnosticSeverity.Warning));
    }
}
function validateAutoLayoutMode(document, expressionRaw, expressionStartOffset, diagnostics) {
    const autoLayoutCalls = parseAutoLayoutCalls(expressionRaw);
    for (const call of autoLayoutCalls) {
        ...
        diagnostics.push(new vscode.Diagnostic(offsetsToRange(document, argStart, argEnd), 'autoLayout(mode) only allows `auto` or `stacked` for the mode value.', vscode.DiagnosticSeverity.Warning));
    }
}
function parseAutoLayoutCalls(source) {
    const calls = [];
...
function getTrimmedBounds(source) {
...
function normalizeAutoLayoutMode(rawValue) {
...
function isAutoLayoutModeToken(token, tokenStart, autoLayoutCalls) {
    const normalizedToken = token.toLowerCase();
...
function maybeAutoCloseBlock(event) {
    ...
    const cursor = new vscode.Position(change.range.start.line, change.range.start.character + change.text.length);
    ...
    const blockType = openBlockMatch[1];
    ...
    const existingTextAfterCursor = document.getText(new vscode.Range(cursor, document.lineAt(document.lineCount - 1).range.end));
    if (new RegExp(`\\{\\{\\s*(?:\\/${blockType}|end[-_]${blockType})\\s*\\}\\}`).test(existingTextAfterCursor)) {
        return;
    }
...
function getSegmentAtPosition(document, position) {
    const segments = getParsleySegments(document.getText());
    const offset = document.offsetAt(position);
    return segments.find((segment) => offset >= segment.innerStartOffset && offset <= segment.fullEndOffset - 2);
}
function getParsleySegments(text) {
    const segments = [];
    const matcher = /\{\{([\s\S]*?)\}\}/g;
...
function isInsideQuotedString(source, index) {
    let inSingle = false;
    let inDouble = false;
...
function getIdentifierAtOffset(source, offset) {
...
function inferVariableDoc(token) {
...
function resolveTokenDocumentation(token) {
    if (VARIABLE_DOCS[token]) {
        return VARIABLE_DOCS[token];
    }
    if (ROOT_DOCS[token]) {
...
function isBraceAccessorRoot(source, tokenStart, token) {
    const afterToken = source.slice(tokenStart + token.length);
    return /^\s*\.\s*\{/.test(afterToken);
}
function isChainedAccessorToken(source, tokenStart) {
...
function isFunctionCallToken(source, tokenStart, token) {
    const afterToken = source.slice(tokenStart + token.length);
    return /^\s*\(/.test(afterToken);
}
function isInsideBraceAccessor(source, index) {
    let inSingle = false;
    let inDouble = false;
    let braceDepth = 0;
    for (let i = 0; i < index; i += 1) {
...
function getHtmlScriptRanges(content) {
    const ranges = [];
...
async function activateLegacyFileCommands(context) {
    try {
        const legacyModulePath = path.join(__dirname, '..', 'src', 'extension.js');
        const legacyModule = require(legacyModulePath);
...
function isWebengineExtensionlessFile(document) {
...
function hasParsleySyntax(content) {
    return /\{\{\s*(?:if|each|else(?:\s+if|-if|_if)?|\/if|\/each|end[-_](?:if|each)|include|meta|set_var|autoLayout\s*\(|!\s*this\b|this\b|meta\.|page\.|globals\.|navigation\.|instance\.|site\.|request\.|settings\.|get_var\.|post_var\.|api\.|[$@][A-Za-z_])[\s\S]*?\}\}/.test(content);
}
function offsetsToRange(document, startOffset, endOffset) {
